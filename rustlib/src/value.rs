...
impl Value {
    ...
    pub fn relu(&self) -> Value {
        ...
        let prop_fn: ProgagateFn = |value| {
            let mut previous = value.previous[0].borrow_mut();
            if value.data > 0.0 {previous.gradient += value.gradient} ;
        };
...
type ProgagateFn = fn(value: &Ref<ValueInternal>);
